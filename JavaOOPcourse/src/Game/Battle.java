...
public class Battle {
    ...
    public Warrior run(){
        while (true){
            ...
            System.out.printf("Game.Hero.Warrior %s give harm %d\n Game.Hero.Warrior %s %d HP\n", attacker.getName(), attack, definder.getName(), definder.getHp());
            if (!definder.isAlive()) {
                System.out.printf("Game.Hero.Warrior %s is dead, %s winner", definder.getName(), attacker.getName());
                return attacker;
            }
            attack = definder.harm();
            attacker.reduceHp(attack);
            System.out.printf("Game.Hero.Warrior %s return harm %d\n Game.Hero.Warrior %s %d HP\n", definder.getName(), attack, attacker.getName(), attacker.getHp());
            if (!attacker.isAlive()) {
                System.out.printf("Game.Hero.Warrior %s is dead, %s winner", attacker.getName(), definder.getName());
                return definder;
            }
        }
    }
